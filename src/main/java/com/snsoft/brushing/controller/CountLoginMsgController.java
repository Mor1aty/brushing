...
import com.snsoft.brushing.service.CountLoginMsgService;
 ...
 * @version 1.0
 * 
 * @date 2018年10月26日下午7:29:53
 * 
 * @Description TODO 统计登录信息接口Controller
 */
@Controller
public class CountLoginMsgController {
	@Resource
	private CountLoginMsgService service;

	@RequestMapping(value = "countLoginMsg.do", method = RequestMethod.POST)
	public void countLoginMsg(HttpServletRequest request, HttpServletResponse response) throws IOException {
		String result = "";
				...
				if (user == null) {
					...
					if (user.getU_identity() == 1) {
						...
						if(correctStart && correctEnd) {
							result = JsonUtil.jsonResponse(service.getLoginMsg(params.get("account").toString(), start, end), AllConstant.CODE_SUCCESS, "查找成功");
						}else {
							if(!correctStart) {
