...
import com.snsoft.brushing.service.CountLanguageMsgService;
 ...
 * @version 1.0
 * 
 * @date 2018年10月26日下午8:20:55
 * 
 * @Description TODO 统计语言信息接口Controller
 */
@Controller
public class CountLanguageMsgController {
	@Resource
	private CountLanguageMsgService service;

	@RequestMapping(value = "countLanguageMsg.do", method = RequestMethod.POST)
	public void countLanguageMsg(HttpServletRequest request, HttpServletResponse response) throws IOException {
		String result = "";
				...
				if (user == null) {
					...
					if (user.getU_identity() == 1) {
						...
						if (correctStart && correctEnd) {
							result = JsonUtil.jsonResponse(service.getLanguageMsg(start, end), AllConstant.CODE_SUCCESS,
									"查找成功");
						} else {
							if (!correctStart) {
