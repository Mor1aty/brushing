...
import com.snsoft.brushing.service.CountAccountLanguageService;
 ...
 * @version 1.0
 * 
 * @date 2018年10月26日下午8:57:02
 * 
 * @Description TODO 统计个人语言信息接口Controller
 */
@Controller
public class CountAccountLanguageController {
	@Resource
	private CountAccountLanguageService service;

	@RequestMapping(value = "countAccountLanguage.do", method = RequestMethod.POST)
	public void countAccountLanguage(HttpServletRequest request, HttpServletResponse response) throws IOException {
		String result = "";
		HashMap<String, String> params = JsonUtil.requestToMap(request);
		result = HttpUtil.checkParams(params, new String[] { "account", "start", "end" });
		// 校验不通过，适用于需要请求参数的情况
				...
				if (user == null) {
					...
					if (user.getU_identity() == 1) {
						...
						if (correctStart && correctEnd) {
							result = JsonUtil.jsonResponse(service.getAccountLanguageMsg(start, end, params.get("account").toString()), AllConstant.CODE_SUCCESS,
									"查找成功");
						} else {
							if (!correctStart) {
